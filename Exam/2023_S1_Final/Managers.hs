...
getAllEmployees (Node left employee right) = getAllEmployees right ++ getAllEmployees left ++ [getName employee]

getName :: Employee -> String
getName (Employee name _) = name
...
totalLowestEmployees tree = length (filter (not . isManager tree) (getAllEmployees tree))

isManager :: BinaryTree Employee -> String -> Bool
isManager Null _ = False
isManager (Node left employee right) name
  | name == getName employee = True
  | otherwise = isManager left name || isManager right name
