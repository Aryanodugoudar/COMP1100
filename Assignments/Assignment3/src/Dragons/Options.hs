...
import           Dragons.Game (DebugFlag(..), GenericAI, MoveSource(..))
import           Options.Applicative 
import           System.Exit (exitFailure)
import           System.IO (hPutStrLn, stderr)
import           Text.Read (readMaybe)
import           Dragons.SushiGo (notPossibleSeed)
...
type AITable st mv = [(String, GenericAI st mv)]
...
data DisplayUI = Text | Json deriving (Eq, Show)
...
-- Please give any non-zero seed!
seed :: Parser Int
seed = option auto $ mconcat
  [long "seed"
    , help "Run the game with a seed to shuffle with"
    , metavar "SEED"
    , value notPossibleSeed
    , showDefault
  ]


  -- where
  --   readSeedUI = maybeReader $ \case
  --     x -> readMaybe x
